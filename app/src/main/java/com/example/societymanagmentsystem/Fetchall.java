
package com.example.societymanagmentsystem;

import com.google.gson.annotations.Expose;
import com.google.gson.annotations.SerializedName;

public class Fetchall {

    @SerializedName("0")
    @Expose
    private String _0;
    @SerializedName("id")
    @Expose
    private String id;
    @SerializedName("1")
    @Expose
    private String _1;
    @SerializedName("name")
    @Expose
    private String name;
    @SerializedName("2")
    @Expose
    private String _2;
    @SerializedName("email")
    @Expose
    private String email;
    @SerializedName("3")
    @Expose
    private String _3;
    @SerializedName("phoneno")
    @Expose
    private String phoneno;
    @SerializedName("4")
    @Expose
    private String _4;
    @SerializedName("image")
    @Expose
    private String image;
    @SerializedName("5")
    @Expose
    private String _5;
    @SerializedName("role")
    @Expose
    private String role;
    @SerializedName("6")
    @Expose
    private String _6;
    @SerializedName("city")
    @Expose
    private String city;
    @SerializedName("7")
    @Expose
    private String _7;
    @SerializedName("s_name")
    @Expose
    private String sName;
    @SerializedName("8")
    @Expose
    private String _8;
    @SerializedName("block")
    @Expose
    private String block;
    @SerializedName("9")
    @Expose
    private String _9;
    @SerializedName("persue")
    @Expose
    private String persue;
    private boolean expanded;
    public Fetchall(String name,String phoneno,String email,String role ,String image ,String city,String persue,String sName,String block){
        this.name=name;
        this.email=email;
        this.phoneno=phoneno;
        this.image= image;
        this.role=role;
        this.city=city;
        this.sName=sName;
        this.block=block;
        this.persue=persue;

        this.expanded=false;
    }

    public String get0() {
        return _0;
    }

    public void set0(String _0) {
        this._0 = _0;
    }

    public String getId() {
        return id;
    }

    public void setId(String id) {
        this.id = id;
    }

    public String get1() {
        return _1;
    }

    public void set1(String _1) {
        this._1 = _1;
    }

    public String getName() {
        return name;
    }

    public void setName(String name) {
        this.name = name;
    }

    public String get2() {
        return _2;
    }

    public void set2(String _2) {
        this._2 = _2;
    }

    public String getEmail() {
        return email;
    }

    public void setEmail(String email) {
        this.email = email;
    }

    public String get3() {
        return _3;
    }

    public void set3(String _3) {
        this._3 = _3;
    }

    public String getPhoneno() {
        return phoneno;
    }

    public void setPhoneno(String phoneno) {
        this.phoneno = phoneno;
    }

    public String get4() {
        return _4;
    }

    public void set4(String _4) {
        this._4 = _4;
    }

    public String getImage() {
        return image;
    }

    public void setImage(String image) {
        this.image = image;
    }

    public String get5() {
        return _5;
    }

    public void set5(String _5) {
        this._5 = _5;
    }

    public String getRole() {
        return role;
    }

    public void setRole(String role) {
        this.role = role;
    }

    public String get6() {
        return _6;
    }

    public void set6(String _6) {
        this._6 = _6;
    }

    public String getCity() {
        return city;
    }

    public void setCity(String city) {
        this.city = city;
    }

    public String get7() {
        return _7;
    }

    public void set7(String _7) {
        this._7 = _7;
    }

    public String getSName() {
        return sName;
    }

    public void setSName(String sName) {
        this.sName = sName;
    }

    public String get8() {
        return _8;
    }

    public void set8(String _8) {
        this._8 = _8;
    }

    public String getBlock() {
        return block;
    }

    public void setBlock(String block) {
        this.block = block;
    }

    public String get9() {
        return _9;
    }

    public void set9(String _9) {
        this._9 = _9;
    }

    public String getPersue() {
        return persue;
    }

    public void setPersue(String persue) {
        this.persue = persue;
    }

    public boolean isExpanded() {
        return expanded;
    }

    public void setExpanded(boolean expanded) {
        this.expanded = expanded;
    }
}
